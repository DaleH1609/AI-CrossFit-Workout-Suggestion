...
// Booking window: per-gym setting (booking_advance_hours). Tests use a fixed
// 48-hour window to mirror the real-world default most gyms use.
const ADVANCE_MS = 48 * 60 * 60 * 1000
...
describe('booking advance window', () => {
  ...
  it('is open for a class starting in exactly 48 hours', () => {
    const atWindow = new Date(Date.now() + ADVANCE_MS).toISOString()
    expect(isBookingWindowOpen(atWindow)).toBe(true)
  })

  it('is NOT open for a class starting more than 48 hours from now', () => {
